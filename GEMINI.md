...
## Agent Protocols & Verification Guidelines

### For AI Agents Working on This Codebase

**Verification Protocols:**
Since this project uses minimal testing, agents must follow specific verification steps:

1. **Weather Algorithm Changes:**
   - When modifying `weather.js`, cross-reference `interpretWetness` outputs with `docs/trail-wetness.md`
   - Verify NJ calibration values match documented thresholds (7:1 snow ratio, clay-rich soil thresholds)
   - Test with real weather data in browser, check tooltip outputs

2. **Calculator Logic Changes:**
   - Always run `npm test` after modifying `calculator.js`
   - Verify edge cases: previous day rollover, midnight boundaries, time formatting
   - Check that `toMinutes`, `fromMinutes`, `format12` functions work correctly

3. **API Integration Changes:**
   - Test with real API calls (not mocks)
   - Verify caching behavior (1 hour for weather, LRU for dawn)
   - Check error handling: network failures, invalid responses, rate limits
   - Test graceful degradation when APIs are unavailable

4. **UI/State Changes:**
   - Manual browser testing required
   - Verify localStorage persistence (save/load cycle)
   - Check responsive design (mobile/desktop)
   - Test state updates trigger correct UI changes

**Agent Modification Guidelines:**
Explicit architectural constraints for agents:

- **No Build Steps**: Maintain vanilla JS/ES6 modules. Do not introduce bundlers, transpilers, or build tools.
- **Consolidated Modules**: Keep related logic in single files (e.g., all weather logic in `weather.js`). Do not fragment into subdirectories.
- **Use `runWhenIdle`**: For initialization tasks that aren't critical for first paint
- **Section Markers**: Use `// === SECTION NAME ===` comments to organize code within large files
- **Input Validation**: Always validate inputs with descriptive error messages
- **Error Handling**: Use `Promise.allSettled` for parallel operations, implement graceful degradation
- **Memory Management**: Clean up timeouts, event listeners, implement cache limits (LRU eviction)
...
### Testing
- Write tests for new features (only pure math/logic like calculator.js)
- Maintain existing test coverage
- Manual testing for features/UI
- AI verification for code correctness

### Performance
- Minimize API calls with caching
- Use local storage efficiently
- Optimize for mobile devices
- Keep bundle size minimal (no build step = no bundle)
