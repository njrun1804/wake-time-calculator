...
## Current Model (v2) - Monmouth County NJ Calibration

**Note:** This model has been calibrated specifically for Monmouth County, NJ coastal conditions (clay-rich soil, moderate drainage, maritime snow patterns). Thresholds are lower than generic models due to local soil saturation characteristics.
...
- **Net moisture balance:** total rainfall + snowmelt (after converting snowfall depth to SWE at a **7:1 ratio for NJ coastal maritime snow**) minus a seasonal share of FAO evapotranspiration. The UI surfaces both the raw ET₀ total and the exact percentage applied so you can confirm the drying adjustment.
- **Monmouth County Calibration:** Drying coefficients tuned for NJ coastal climate:
  - Base coefficient: 0.5
  - Summer (Jun-Aug): 0.75/day (faster drying, warm)
  - Winter (Nov-Mar): 0.92/day (slow drying, cloudy/humid)
  - Spring/Fall (Apr-May, Sep-Oct): 0.85/day (moderate)
- **Heavy events:** any day with a balance ≥ 1.0" is treated as a saturation event (calibrated for clay-rich soil with moderate drainage).
...
| `Slick`       | ≥0.20" in 72h, ≥3 wet days, net ≥0.35", or freeze-thaw _with_ recent liquid | Dial cornering, expect greasy tread  |
| `Muddy`       | ≥0.35" in 48h, heavy event, or net ≥0.6"                                    | Poles/gaiters recommended            |
| `Soaked`      | ≥0.5" in 24h or net ≥1.3"                                                   | Expect standing water, big slowdowns |
